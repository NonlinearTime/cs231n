...
def batchnorm_backward(dout, cache):
    ...
    dx_norm = gamma * dout
    dvar = np.sum(- 0.5 * dx_norm *(x - sample_mean) * np.power(sample_var + eps,-1.5),axis = 0)
    dmean = np.sum(dx_norm * ( - np.power(sample_var + eps,-0.5)),axis = 0) + dvar * np.sum(-2 * (x - sample_mean),axis = 0)/ m
    dx = dx_norm * np.power(sample_var + eps,-0.5) + 2 * dvar *(x - sample_mean) * np.ones_like(x) / m + dmean * np.ones_like(x) / m
...
def conv_forward_naive(x, w, b, conv_param):
    ...
    out = np.ones((N,F,new_H,new_W))
    
    for i in range(new_H):
        for j in range(new_W):
            flr = x_padded[:,:,i * stride: i * stride + HH,j * stride: j * stride + WW]
            #print(flr.shape)
            for k in range(F):
                out[:,k,i,j] = np.sum(flr * w[k,:,:,:],axis = (1,2,3))
...
def conv_backward_naive(dout, cache):
    ...
    for i in range(H_out):
        for j in range(W_out):
            flr = x_padded[:,:,i * stride: i * stride + HH,j * stride: j * stride + WW]
            for k in range(F):
                dw[k,:,:,:] += np.sum((dout[:,k,i,j])[:, None, None, None] * flr,axis = 0)
            for n in range(N):
...
def max_pool_forward_naive(x, pool_param):
    ...
    H1 = (int)((H - pool_height) / stride + 1)
    W1 = (int)((W - pool_width) / stride + 1)
    out = np.zeros((N,C,H1,W1))
    
    for i in range(H1):
        for j in range(W1):
            mask = x[:,:,i * stride: i * stride + H1,j * stride: j * stride + W1]
