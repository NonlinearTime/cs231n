...
class CaptioningRNN(object):
    ...
    def loss(self, features, captions):
        ...
        loss, dx = temporal_softmax_loss(aff_out, captions_out, mask, verbose=False)

        daff_out , grads["W_vocab"], grads["b_vocab"] = temporal_affine_backward(dx, aff_cache)
        if self.cell_type == "rnn":
            dfw_out, dh0, grads["Wx"], grads["Wh"], grads["b"] = rnn_backward(daff_out, forward_cache)
        else:
            pass
        grads["W_embed"] = word_embedding_backward(dfw_out, embed_cache)
